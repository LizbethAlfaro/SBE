<?php
	...
	if (empty($_POST['rut'])) {
		...
		$actividad= intval($_POST["actividad"]);
                ...
                $query= Integrante::recuperarIntegrante($rut_estudiante,$rut,$con,"","","");
		$count=sqlsrv_num_rows($query);
                
                $query_new_insert=false;
                //valida si existe el integrante previamente
		if ($count==0){ //modificado ya que decia == 0
                 $query_new_insert = Integrante::registrarIntegrante($rut_estudiante,$rut,$nombre,$apellido,$genero,$fechaNac,$relacion,$estadoCivil,$nivelEduc,$actividad,$prevision,$otraPrevision,$condicion,$enfermedad,$con);      

                        }
                
		
			if ($query_new_insert) {
        $messages[] = "Integrante ha sido ingresado satisfactoriamente.";
        ...
        $interes_mensual = 0;
        $ganancia_mensual = 0;
        $pension_alim_mensual = 0;
        $actividad_mensual = 0;
        Ingreso::registrarIngreso($rut_estudiante, $rut, $nombre, $apellido, $sueldo_mensual, $pension_mensual, $honorario_mensual, $retiro_mensual, $dividendo_mensual, $interes_mensual, $ganancia_mensual, $pension_alim_mensual, $actividad_mensual, $con);



        $accion = "Registro integrante $rut a estudiante $rut_estudiante";
			} else{
				$errors []= "Ya ha sido ingresado el integrante";
                                    $accion = "Error al registrar integrante $rut a estudiante $rut_estudiante";
			}
