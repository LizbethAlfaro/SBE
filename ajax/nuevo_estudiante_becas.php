...
//verificar si es postulado o matriculado
// 0 no registrado
// 1 postulado
// 2 matriculado
$matri_post = UMAS::matriculadoPostulado($rut,$con);

$informacion;

switch ($matri_post){
  case 1:
      $informacion[] = " Su estado actual es Postulado, ";
      break;
  case 2:
      $informacion[] = " Su estado actual es Matriculado, ";
      break;
  default :
      $informacion[] = " No hay Registro ";
      break;
...
//verificar si ya ha postulado a la beca socieconomica
$condicion="";    
$result_socieconomica = Estudiante::recuperarEstudiante($rut_3, $con,$condicion,"","");

if($result_socieconomica){
  $contador_socieconomica= sqlsrv_num_rows($result_socieconomica); 
  
}else{
  $contador_socieconomica=0;  
}

if($contador_socieconomica>0 && $beca!='2'){
  //and $beca!="2" 
  $errors[] = "Ya cuenta con una postulacion a la beca socieconomica, el beneficio seleccionado no es compatible con ella.";  
    
}else{
//verificar si es renovante o postulante
$result_renoPost = UMAS::renovantePostulante($rut,$beca,$con);

if($result_renoPost){
$contador_renoPost = sqlsrv_num_rows($result_renoPost);
$renoPostArreglo=[];
if($contador_renoPost>0){    
 while ($renoPostCursor = sqlsrv_fetch_array($result_renoPost)) {
 $renoPostArreglo     = array(
     "ANO"          => $renoPostCursor['ANO'],
    );
}
$renoPost=2;  
}else{
...
//validar estudiante
$result = UMAS::recuperarEstudianteBecaInterna($rut,$beca,$renoPost,$con);
...
//        $insert_solicitud= Solicitud::registrarSolicitud($rut,$estado,$tipo,$con);
   ...
   $errors[] = " No esta habilitado para este beneficio "; 
}

}else{
    
   $errors[] = " Proceso Cerrado "; 
}
