...
function obtenerAcreditacion($rut,$con){
ob_start();
//require_once ($_SERVER['DOCUMENT_ROOT']."/BecasBeneficios/admin/acreditarObtenerInforme.php");
require_once ($_SERVER['DOCUMENT_ROOT']."/admin/acreditarObtenerInforme.php");
$html = ob_get_clean();
//echo $html;
//echo $title;
...
//date("Y-m-d H:i:s")
$nombreArchivo = $rut."_"."A"."_".date("d-m-Y").".pdf";
$tipo="Formulario ASE Acreditado";
$options = new Options();
$options->setDpi(180);
...
if (!file_exists($rutaGuardado.$nombreArchivo)) {
  ...
  //  echo 'registro acreditacion';
}else{
    
    $errors[]= "Informe ya fue acreditado";
    if (isset($errors)){
